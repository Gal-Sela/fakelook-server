...
namespace fakeLook_starter.Services
{
    public class DtoConverter : IDtoConverter
    {
        ...
        {
            return new Post() { Id = post.Id, Date = post.Date, Description = post.Description, ImageSorce = post.ImageSorce, X_Position = post.X_Position, Y_Position = post.Y_Position, Z_Position = post.Z_Position, UserId = post.UserId };
        }
